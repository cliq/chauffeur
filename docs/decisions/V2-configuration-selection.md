...
Build each child environment separately from the login environment. Strip provider authentication/routing variables, inherited Codex/Claude identity, tmux identity, and shell/loader injection variables. Resolve harness directories from the project team, then apply the selected agent preset’s optional fixed configuration directory for its harness. Apply the resolved directories with Chauffeur session values for that child. Global presets offer “Use team configuration” (the default) or “Use a specific directory”; the editor expands `~`, so a preset such as Claude Code Local can use `~/.claude-local`. Custom team copies preserve this choice independently. Team selection still determines which presets are available. Base presets configure commands; independent custom copies can override launch arguments. Blank team paths use harness defaults. Never mutate the app's global environment or create a native configuration directory.
