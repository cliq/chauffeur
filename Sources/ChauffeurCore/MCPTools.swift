...
public enum MCPTools {
    ...
    public static var definitions: [JSONValue] {
        ...
        return [
            ...
            tool("chauffeur_inbox", "Read your own queued/received messages, optionally wait up to 300 seconds (use 300 while orchestrating), and acknowledge IDs after processing them. Busy or exited recipients retain their inbox. Arrival wakes the wait immediately. An empty response means timeout or a worker state change: check delegation status, then wait again if still active. Waiting does not submit terminal input.", ["waitSeconds": .object(["type": .string("integer"), "minimum": .number(0), "maximum": .number(300)]), "acknowledge": strings], []),
